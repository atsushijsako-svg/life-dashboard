...
export default function App() {
  ...
  const filledCount = [
    ...
    form.散歩, form.朝ストレッチ, form.夜ストレッチ, form.水2L, form.体重
  ].filter(v => v && v !== "").length;
      ...
      <div style={{ padding: "14px 14px 0" }}>
          ...
          <div>
            ...
            <div style={{ background: c.card, borderRadius: 14, padding: "12px 16px", marginBottom: 12, border: `1px solid ${c.border}` }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                ...
                <div style={{ fontSize: 11, color: filledCount === 10 ? c.green : c.muted, fontWeight: filledCount === 10 ? 700 : 400 }}>
                  {filledCount === 10 ? "✅ 全項目入力済み" : `${filledCount} / 10 項目入力済み`}
                </div>
              </div>
              {/* 進捗バー */}
              <div style={{ background: c.section, borderRadius: 6, height: 6, overflow: "hidden" }}>
                <div style={{ width: `${Math.round((filledCount / 10) * 100)}%`, height: "100%", background: filledCount === 10 ? c.green : c.accent, borderRadius: 6, transition: "width 0.3s" }} />
              </div>
              {/* 自動保存メッセージ */}
              {autoSaveMsg && (
                <div style={{
            ...
            <Card bg={c.healthBg}>
              ...
              <div style={{ marginBottom: 14 }}>
                <FL>散歩</FL>
                <SimpleToggle fieldKey="散歩" val={form.散歩} onToggle={handleToggle} />
              </div>
        ...
        {tab === 1 && (
          <div>
            ...
            <Card>
              <ST>達成率</ST>
              {["発信","営業","筋トレ","水泳","食事","散歩","朝ストレッチ","夜ストレッチ","水2L"].map(key => {
                const r = rate(key);
              ...
              {data.length === 0 ? (
                ...
                <div key={i} style={{ paddingBottom: 12, marginBottom: 12, borderBottom: i < 4 ? `1px solid ${c.border}` : "none" }}>
                  ...
                  <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
                    {["発信","営業","筋トレ","水泳","食事","散歩","朝ストレッチ","夜ストレッチ","水2L"].map(k => (
